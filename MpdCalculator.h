...
private:
  ...
  TH1F	*h_pt_PID_efficiency_after[NetaBins];
  TH1F	*h_pt_PID_efficiency_after_sorts[NetaBins][_N_SORTS];

  TH2F* h2_flow_wrt_RP_vs_pt[NcentralityBinsFlow][_N_HARM];
  TH2F* h2_flow_wrt_full_vs_pt_divided[NcentralityBinsFlow][_N_HARM][_N_HARM][_N_METHOD];
  TH2F* h2_b_vs_multiplicity;
  ...
  TProfile *p_flow_GEN_RP_vs_centrality[_N_HARM], *p_flow_GEN_RP_vs_pt[NcentralityBinsFlow][_N_HARM], *p_flow_GEN_RP_vs_eta[NcentralityBinsFlow][_N_HARM], *p_flow_GEN_RP_vs_rapidity[NcentralityBinsFlow][_N_HARM];
...
public:
  ...
  void FillTPC(Int_t centrality_bin, EPParticle *event_plane_buffer, Int_t ep_particle_count, Float_t weightB);
  void FillZDC(Int_t centrality_bin, Float_t *ZDC_energy_mpd, Float_t weightB);
  ...
  Int_t GetCent10Bimp(Float_t bimp);
};

Int_t MpdCalculator::GetCent10Bimp(Float_t bimp)
{
    int fcent;
    if     ( bimp<4.5 )  fcent = 0; // 0-10%
    else if( bimp<6.3 )  fcent = 10; //10-20%
    else if( bimp<7.73 ) fcent = 20; //20-30%
    else if( bimp<8.92 ) fcent = 30; //30-40%
    else if( bimp<9.99)  fcent = 40; //40-50%
    else if( bimp<10.83) fcent = 50; //50-60%
    else if( bimp<11.78) fcent = 60; //60-70%
    else if( bimp<12.61) fcent = 70; //70-80%
    else                 fcent =-1;
